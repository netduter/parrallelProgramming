...
void Exercise2::print_calculated_results() {
    for(int i = 0; i < nprocs - 1; i++)
        MPI_Recv(msg_buffer, MAX_BUFFER_SIZE, MPI_CHAR, i + 1,
...
void Exercise2::read_file() {
        ...
        if(line.length() == dna_seq.length()) {
            ...
            if(MAX_BUFFER_SIZE - msg.length() >
               id.length() + dna_seq.length() + 1) {
                ...
                MPI_Bcast(msg_buffer, MAX_BUFFER_SIZE, MPI_CHAR, 0,
                          MPI_COMM_WORLD);
    ...
    for(int i = 0; i < how_many_times_print_calculated_results; i++) {
